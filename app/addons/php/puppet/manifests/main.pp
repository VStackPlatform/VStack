...
if $php['install'] == true {
  ...
  if $apache != false and $apache['options']['mpm'] == 'prefork' {
  ...
  if $php['options']['xdebug'] == true {
    php::module { 'xdebug':
      module_prefix => "php5-", #All php packages are prefixed by php5 in ubuntu.
    }
    each( $php['options']['xdebug_options']['settings'] ) |$key, $setting| {
      php::augeas { $key:
        entry       => "XDEBUG/${key}",
        value       => $setting,
  ...
  ## Install PHP modules.
  each( $php['options']['modules'] ) |$module| {
    php::module { $module:
      module_prefix => "php5-", #All php packages are prefixed by php5 in ubuntu.
    }
  }
  ...
  each( $php['options']['pear'] ) |$module| {
    php::pear::module { $module: }
  }

  ## Install PECL modules.
  each( $php['options']['pecl'] ) |$module| {
    php::pecl::module { $module: }
  }

  ## Add INI settings.
  each( $php['options']['ini_settings'] ) |$key, $setting| {
    php::augeas { $key:
      entry       => "CUSTOM/${key}",
      value       => $setting,
      target      => $phpIni,
